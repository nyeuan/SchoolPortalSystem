<?php
// File: professor/edit-assignment.php
...
try {
    ...
    if (!$existing) { header('Location: manage-course.php?course_id=' . $course_id . '&error=not_authorized'); exit; }
    ...
    if (isset($_FILES['attachment_file']) && $_FILES['attachment_file']['error'] === UPLOAD_ERR_OK) {
        ...
        if (!in_array($extension, $allowed_extensions, true)) { header('Location: manage-course.php?course_id=' . $course_id . '&error=invalid_file_type'); exit; }

        $upload_dir = __DIR__ . '/../public/uploads/assignments/';
        if (!is_dir($upload_dir)) { mkdir($upload_dir, 0755, true); }

        $stored_filename  = uniqid('asg_', true) . '.' . $extension;
        $destination_path = $upload_dir . $stored_filename;

        if (!move_uploaded_file($tmp_path, $destination_path)) { header('Location: manage-course.php?course_id=' . $course_id . '&error=upload_failed'); exit; }

        if ($existing['AttachmentPath']) { $old_attachment_to_delete = $existing['AttachmentPath']; }
    ...
    $update_stmt = $pdo->prepare("UPDATE Assignments SET Title = :title, Description = :description, DueDate = :due_date, MaxScore = :max_score, AttachmentName = :attachment_name, AttachmentPath = :attachment_path WHERE Assignment_ID = :assignment_id");
    $update_stmt->execute([':title' => $title, ':description' => $instructions, ':due_date' => $due_date_formatted, ':max_score' => $max_score, ':attachment_name' => $attachment_name, ':attachment_path' => $attachment_path, ':assignment_id' => $assignment_id]);

    if ($old_attachment_to_delete && file_exists(__DIR__ . '/../public/' . $old_attachment_to_delete)) { unlink(__DIR__ . '/../public/' . $old_attachment_to_delete); }
