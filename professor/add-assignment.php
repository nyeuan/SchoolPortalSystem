<?php
// File: professor/add-assignment.php
...
try {
    $insert_stmt = $pdo->prepare("INSERT INTO Assignments (Title, Description, DueDate, MaxScore, AttachmentName, AttachmentPath, FK_CourseModule_ID) VALUES (:title, :description, :due_date, :max_score, :attachment_name, :attachment_path, :course_module_id)");
    $insert_stmt->execute([':title' => $title, ':description' => $instructions, ':due_date' => $due_date_formatted, ':max_score' => $max_score, ':attachment_name' => $attachment_name, ':attachment_path' => $attachment_path, ':course_module_id' => $course_module_id]);
} catch (PDOException $e) {
    if ($attachment_path && file_exists(__DIR__ . '/../public/' . $attachment_path)) { unlink(__DIR__ . '/../public/' . $attachment_path); }
