...
try {
    // FIXED: Shifted to strict lowercase table selectors to match your forward-engineered environment rules
    ...
    // Fetch dynamic grade statistics using the new direct FK link on the lowercase table structure
    $grade_stmt = $pdo->query("
        ...
        LEFT JOIN courses c ON sec.Section_ID = c.FK_Section_ID
    ...
    // ── Build Dynamic Query using direct Section column ────────────────
    $where_clauses = [];
    $params = [];
    ...
    // Count records directly from lowercase courses table context
    ...
    // Fetch independent course records directly mapped to sections
    $course_query = "
        SELECT 
...
$success_messages = [
    'course_added'    => 'Course record successfully created and assigned independently to the section.', //
    'course_deleted'  => 'Course record removed from database index storage.', //
    'section_created' => 'New section directory lane added successfully.',
];
$error_messages = [
    'missing_fields' => 'Please fill in all fields with valid configurations.', //
    'delete_failed'  => 'Foreign key restriction: Cannot delete course with dependencies.', //
];
...
<body class="bg-gradient-to-br from-school-green via-[#125730] to-school-yellow min-h-screen font-serif text-gray-800 flex flex-col md:flex-row">
    ...
    <main class="ml-0 md:ml-64 flex-1 p-4 sm:p-8 overflow-y-auto h-screen w-full flex flex-col">
        ...
        <section class="bg-[#fcfbf7] rounded-3xl p-6 shadow-lg border border-school-gold/20 mb-6 shrink-0">
            <div class="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                <div>
                    ...
                    <p class="text-gray-500 italic mt-1">Viewing: <?= htmlspecialchars($current_grade_name) ?></p>
                </div>
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    <?php endif; ?>
                    <button onclick="document.getElementById('addSectionModal').classList.remove('hidden')" class="bg-school-gold text-white px-5 py-2.5 rounded-2xl font-semibold hover:opacity-90 transition text-xs font-sans whitespace-nowrap">+ Add Section</button>
                    <button onclick="document.getElementById('addCourseModal').classList.remove('hidden')" class="bg-school-green text-white px-5 py-2.5 rounded-2xl font-semibold hover:bg-school-green-hover transition text-xs font-sans whitespace-nowrap">+ Add Course</button>
                </div>
            </div>

            <div class="mt-6 border-t border-gray-100 pt-4">
                <form action="admin-manage-course.php" method="GET" class="flex gap-2 font-sans">
                    <?php if ($selected_grade_id > 0): ?><input type="hidden" name="grade_level_id" value="<?= $selected_grade_id ?>"><?php endif; ?>
                    <input type="text" name="search" value="<?= htmlspecialchars($search) ?>" placeholder="Search directory globally..." class="w-full border border-gray-300 rounded-xl px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-school-green text-sm">
                    <button type="submit" class="bg-school-gold text-white px-6 py-2.5 rounded-xl font-semibold hover:opacity-90 transition text-sm">Search</button>
                </form>
            </div>
        </section>
        ...
        <div class="flex-1 flex flex-col gap-6">
            <div>
                <h2 class="text-xl font-bold text-white mb-3 font-sans flex items-center gap-2">🎓 Academic Tiers</h2>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 font-sans">
                    <?php foreach ($grade_cards as $card): ?>
                        <a href="admin-manage-course.php?grade_level_id=<?= $card['GradeLevel_ID'] ?>" class="block bg-[#fcfbf7] rounded-2xl p-4 shadow border transition <?= ($selected_grade_id === (int)$card['GradeLevel_ID']) ? 'border-school-green ring-2 ring-school-green/20 bg-emerald-50/20' : 'border-school-gold/15 hover:border-school-green/40 hover:shadow-md' ?>">
                            <h3 class="font-bold text-lg text-school-green"><?= htmlspecialchars($card['GradeName']) ?></h3>
            ...
            <div class="mt-2 flex-1 flex flex-col">
                <h2 class="text-xl font-bold text-white mb-3 font-sans">📚 Courses (Page <?= $page ?> of <?= max(1, $total_pages) ?>)</h2>
                <?php if (empty($courses)): ?>
                    <div class="bg-[#fcfbf7] rounded-3xl p-10 text-center shadow-lg border border-school-gold/20 flex-1 flex items-center justify-center">
                        <p class="text-gray-400 italic font-sans">No course listings found matching this criteria.</p>
                    </div>
                <?php else: ?>
                            ...
                            <div class="bg-[#fcfbf7] rounded-2xl p-5 shadow-md border border-school-gold/10 flex flex-col justify-between font-sans relative">
                                <div>
                                    <div class="flex justify-between items-center mb-3">
                                        <span class="bg-school-green/10 text-school-green font-bold text-xs px-2.5 py-1 rounded-md"><?= htmlspecialchars($course['CourseCode']) ?></span>
                                        <span class="text-xs px-2.5 py-1 rounded-full font-medium <?= $course['Status'] === 'Active' ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-600' ?>"><?= htmlspecialchars($course['Status']) ?></span>
                                    </div>
                                    ...
                                    <h3 class="text-lg font-bold text-school-green font-serif mt-1"><?= htmlspecialchars($course['CourseName']) ?></h3>
                                </div>
                                <div class="mt-6 flex justify-between items-center border-t pt-4 border-gray-100 gap-2">
                                    <a href="admin-course-assignment.php?id=<?= $course['Course_ID'] ?>" class="text-xs font-semibold bg-blue-50 text-blue-600 border border-blue-200 px-3 py-2 rounded-xl hover:bg-blue-100 transition text-center flex-1">Assign Users</a>
                                    <form action="admin-add-course.php" method="POST" class="flex-1" onsubmit="return confirm('Confirm permanent course removal?');">
                                        <input type="hidden" name="action" value="delete">
                                        <input type="hidden" name="course_id" value="<?= (int)$course['Course_ID'] ?>">
                                        <button type="submit" class="w-full text-xs font-semibold bg-red-50 text-red-600 border border-red-200 px-3 py-2 rounded-xl hover:bg-red-100 transition text-center">Delete Course</button>
                                    </form>
                                </div>
                            </div>
                        <?php endforeach; ?>
    ...
    <div id="addSectionModal" class="hidden fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50">
        <div class="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl font-sans">
            ...
            <form action="admin-add-course.php" method="POST">
                ...
                <input type="text" name="section_name" required maxlength="45" class="w-full border border-gray-300 rounded-xl px-4 py-2 mb-4 focus:outline-none focus:ring-2 focus:ring-school-green text-sm" placeholder="e.g., Section A">
                <label class="block text-sm font-semibold text-gray-600 mb-1">Select Academic Tier</label>
                <select name="grade_level_id" required class="w-full border border-gray-300 rounded-xl px-4 py-2 mb-6 focus:outline-none focus:ring-2 focus:ring-school-green bg-white text-sm">
                    ...
                    <?php foreach ($grade_levels_list as $gl): ?>
                        <option value="<?= (int)$gl['GradeLevel_ID'] ?>" <?= ($selected_grade_id === (int)$gl['GradeLevel_ID']) ? 'selected' : '' ?>><?= htmlspecialchars($gl['GradeName']) ?></option>
                    <?php endforeach; ?>
                </select>
                <div class="flex justify-end gap-3">
                    <button type="button" onclick="document.getElementById('addSectionModal').classList.add('hidden')" class="px-4 py-2 rounded-xl text-gray-500 hover:bg-gray-100 text-sm">Cancel</button>
                    <button type="submit" class="bg-school-gold text-white px-5 py-2 rounded-xl font-semibold hover:opacity-90 text-sm">Create Section</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="addCourseModal" class="hidden fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50">
        <div class="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl font-sans max-h-[90vh] overflow-y-auto">
            ...
            <form action="admin-add-course.php" method="POST">
                ...
                <input type="text" name="course_code" required maxlength="45" class="w-full border border-gray-300 rounded-xl px-4 py-2 mb-4 focus:outline-none focus:ring-2 focus:ring-school-green text-sm" placeholder="e.g., ENG-101">
                <label class="block text-sm font-semibold text-gray-600 mb-1">Course Name</label>
                <input type="text" name="course_name" required maxlength="45" class="w-full border border-gray-300 rounded-xl px-4 py-2 mb-4 focus:outline-none focus:ring-2 focus:ring-school-green text-sm" placeholder="e.g., Introduction to Literature">
                <label class="block text-sm font-semibold text-gray-600 mb-1">Assign Academic tier and Section</label>
                <select name="section_id" required class="w-full border border-gray-300 rounded-xl px-4 py-2 mb-4 focus:outline-none focus:ring-2 focus:ring-school-green bg-white text-sm">
                    <option value="" disabled selected hidden>Choose assigned grade level and section...</option>
                    <?php foreach ($sections_list as $sec): ?>
                        <?php 
                            $grade_title = 'Unassigned';
                            foreach ($grade_levels_list as $gl) {
                                if ($gl['GradeLevel_ID'] == $sec['FK_GradeLevel_ID']) { $grade_title = $gl['GradeName']; break; }
                            }
                        ?>
                        <option value="<?= (int)$sec['Section_ID'] ?>"><?= htmlspecialchars($grade_title) ?> &gt; <?= htmlspecialchars($sec['SectionName']) ?></option>
                    <?php endforeach; ?>
                </select>
                <label class="block text-sm font-semibold text-gray-600 mb-1">Set Course Status</label>
                ...
                <div class="flex justify-end gap-3">
                    <button type="button" onclick="document.getElementById('addCourseModal').classList.add('hidden')" class="px-4 py-2 rounded-xl text-gray-500 hover:bg-gray-100 text-sm">Cancel</button>
                    <button type="submit" class="bg-school-green text-white px-5 py-2 rounded-xl font-semibold hover:bg-school-green-hover text-sm">Confirm Changes</button>
                </div>
            </form>
        </div>
    </div>
