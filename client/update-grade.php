<?php
// Handles: Grade form submission from grade-submissions.php
...
if (!$assignment_id || !$submission_id || $score === false) {
    header('Location: grade-submissions.php?assignment_id=' . $assignment_id . '&error=missing_fields');
    exit;
}

try {
    // Confirm this professor actually owns the course this assignment belongs to,
    // and that MaxScore isn't exceeded.
    ...
    if ($score < 0 || $score > $assignment['MaxScore']) {
        header('Location: grade-submissions.php?assignment_id=' . $assignment_id . '&error=invalid_score');
        exit;
    }
    ...
    $update_stmt = $pdo->prepare("
        ...
        WHERE AssignmentSubmission_ID = :submission_id AND FK_Assignment_ID = :assignment_id
    ");
    $update_stmt->execute([
        ':score'         => $score,
